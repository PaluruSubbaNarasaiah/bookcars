 ...
 * @returns {*}
 */
export const currencyRTL = (currencySymbol) => {
 ...
 * @returns {number}
 */
export const calculateTotalPrice = (car, from, to, priceChangeRate, options) => {
    let totalPrice = 0;
 ...
 */
export const convertPrice = async (amount, from, to) => {
 ...
 * @returns {boolean}
 */
export const checkCurrency = (currency) => currencies.findIndex((c) => c === currency) > -1;
/**
 ...
 * @returns {boolean}
 */
export const isEnglish = (language) => language === 'en';
/**
 ...
 * @returns {string}
 */
export const formatDistance = (d, language) => {
    if (d > 0) {
        if (d < 1) {
            d *= 1000;
        }
        const parts = String(d % 1 !== 0 ? d.toFixed(2) : d).split('.');
        const separator = language === 'en' ? ',' : ' ';
        parts[0] = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, separator);
